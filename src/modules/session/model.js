...
const findAll = () => {
  return db
    .execute(
      "SELECT *, session.id FROM session JOIN location ON session.location_id=location.id"
    )
    .then((data) => {
      return data;
    });
...
const createWineHasSession = (data) => {
  ...
  return db.execute(
    "INSERT INTO session_has_wine (wine_id, session_id) VALUES (?, ?)",
    [wineId, sessionId]
  );
};
...
const getUserSessionsBySessionId = (user_id, session_id) => {
  return db.execute(
    "Select * from session_has_user where user_id = ? and session_id =?",
    [user_id, session_id]
  );
};
